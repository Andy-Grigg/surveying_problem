...
ax.set_xticklabels(
    [
        ...
        for (grid_size, threshold), _ in df.iterrows()
    ]
)
ax.set_yscale("log")
...
df["average_well_size"] = df.apply(
    lambda row: row.number_of_sites / row.number_of_wells
    if row.number_of_wells != 0
    else 0,
    axis=1,
)
...
df_recursive_vs_stack_size_high = df_recursive_vs_stack.loc[
    pd.IndexSlice[:, 0.99], columns
]
df_recursive_vs_stack_size_low = df_recursive_vs_stack.loc[
    pd.IndexSlice[:, 0.7], columns
]
df_recursive_vs_stack_prob_big = df_recursive_vs_stack.loc[
    pd.IndexSlice[10000, :], columns
]
df_recursive_vs_stack_prob_small = df_recursive_vs_stack.loc[
    pd.IndexSlice[10, :], columns
]

df_recursive_vs_stack_size_high.index = df_recursive_vs_stack_size_high.index.droplevel(
    1
)
...
df_recursive_vs_stack_prob_small.index = (
    df_recursive_vs_stack_prob_small.index.droplevel(0)
)
...
ax[0].legend(
    labels=[
        "Recusive Method: Threshold = 0.99",
        "Stack Method: Threshold = 0.99",
        "Recusive Method: Threshold = 0.7",
        "Stack Method: Threshold = 0.7",
    ]
)
...
ax[1].legend(
    labels=[
        "Recusive Method: Grid Size = 10,000",
        "Stack Method: Grid Size = 10,000",
        "Recusive Method: Grid Size = 10",
        "Stack Method: Grid Size = 10",
    ]
)
plt.show()
...
df_list_vs_deque_size_high.plot(
    ax=ax[0], title="Dependence of grid size on execution time"
)
...
ax[0].legend(
    labels=[
        "Deque Implementation: Threshold = 0.99",
...
df_list_vs_deque_prob_big.plot(
    ax=ax[1], title="Dependence of probability threshold on execution time"
)
df_list_vs_deque_prob_small.plot(ax=ax[1])

ax[1].legend(
    labels=[
        "Deque Implementation: Grid Size = 10,000",
